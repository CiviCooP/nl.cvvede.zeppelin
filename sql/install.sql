CREATE VIEW `maatjesproject gezinsleden` AS
...
CREATE VIEW `maatjesproject gezinsleden count` AS
select count(id) as count, `gezinslid` from `maatjesproject gezinsleden`
group by `gezinslid`;
...
END as `status`,
case
	WHEN `maatjesproject gezinsleden count`.`count` > 0 THEN 'Ja'
    ELSE 'Nee'
...
LEFT JOIN `maatjesproject gezinsleden count` ON `maatjesproject gezinsleden count`.`gezinslid` = `civicrm_contact`.`id`
